...
\label{intro}
	...
	The Atlantic meridional overturning circulation (AMOC) has been the focus of many studies over the past decades because of its acceptance as representation for ocean circulation and studies showing its correlation to the climate system (Pardaens 2011 ... Meinen refs). While the causal relation between AMOC variability and climatic events (e.g. hurricane activity, rainfall, air temperature) has been disputed \cite{wunschAndHeimbach_AMOC}, understanding the influence of external forcing and the mechanisms responsible for its variability are crucial for understanding its role in the climate system and usefulness in prediction.  

	The AMOC in the North Atlantic has been the subject of numerous studies (e.g. \cite{mcCarthy}, \cite{pillar}, or \cite{meinenSamoc} for a more complete list) because of its proximity to the formation of North Atlantic Deep Water \red{cite Stommel, Stouffer? from Dong SAMOCfromSSH} and the observation arrays in the basin \red{e.g. RAPID cite rayner 2011, wunschpg3}. In contrast, we study the MOC in the South Atlantic (SAMOC) because it sits at the ``mouth'' of the Atlantic, funneling warm, salty water from the neighboring ocean basins to the Northern hemisphere (Biastoch et al. 2008; Lee et al. 2011; Kelly et al., 2014, all from Dong) and distributing cold, salty North Atlantic Deep Water at depth \red{def need refs}. Variability in the region is complicated by rough topography to the South, Agulhas rings and the Benguela current to the East, and the Brazil and Malvinas confluence on the West. \cite{meinenSamoc} investigate high frequency variability in the SAMOC based on a 20 month record of daily mooring observations and show that Ekman processes are responsible for variability on the order of 20 days while the geostrophic component is responsible for variation from 20 to 90 days. They report an average daily SAMOC value of \red{ Sv}  \red{Dong et al} use altimetry derived TS profiles  

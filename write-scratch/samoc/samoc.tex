...
\label{setup}
 ...
 \section{Sensitivity Maps}
  ...
  \red{Proof for why this works?}  
   ...
   \begin{figure}
    \centering
    \includegraphics[width=\textwidth]{reconstruct.34S/full_reconstruct_mean_flux}
    \caption{SAMOC reconstructed as in Eqn. \ref{eq:reconstruct} with monthly accumulated sensitivities and atmospheric forcing due to the wind stress and net heat and salt fluxes. ``Memory'' time is \red{1 year}.}
    \label{fig:fullReconstruction}
   \end{figure}

  \subsection{Exploring reconstruction memory \& buoyancy contributions}

  When reconstructing AMOC as in Eqn. \ref{eq:reconstruct} with only December sensitivities ($\pderiv{\mathcal{J}}{\mathcal{F}_k}$), increasing the memory time significantly changes the quality and phase of the reconstruction (see Fig. \ref{fig:phaseChangeAqh}). \red{This phase change arises because the ocean responds to changes in heat flux related variables differently enough (to be detected) during different times of the year.} See Fig. \ref{fig:bigPlotHflux} for how June and December sensitivities look in different regions of the world. Notably in the area around Greenland and in the Atlantic. See Fig. \ref{fig:bigPlotTauu} for how these sensitivities are almost identical for wind stress. Thus, when performing the heat flux reconstruction with monthly sensitivities (but removing the zeros this time ...), the phase change goes away, only amplitude increases with memory time (see Fig. \ref{fig:aqh}). 
   ...
   \begin{figure}
    \centering
    \includegraphics[width=.7\textwidth]{reconstruct.34S/monthly/aqh_reconstruct}
  ...
  So this seems to imply that it is actually important to compute monthly sensitivities for an accurate reconstruction. However, the reconstruction from wind stress essentially ``nails it''. See Fig. \ref{fig:windReconstruct}. The absolute error between the forward model reconstruction and wind stress reconstruction is \red{indistinguishable from white noise} \ref{fig:windRecError} \red{talk about fourier transform here}. Additionally, when considering the two norm of the reconstruction error from the forward model, the quality only gets worse when buoyancy fluxes are added (Fig. \ref{fig:reconstructError}).  However, when considering the reconstruction at 26N, there seems to be a roughly decadal mode in the error between the two: Fig \ref{fig:windRecError26N}. We can't say anything about adding heat/salt flux here though because I only have December evaluated objective functions completed. 

   \begin{figure}
    \centering
    \includegraphics[width=.7\textwidth]{reconstruct.34S/wind_vs_full}
   ...
   \begin{figure}
    \centering
    \includegraphics[width=.7\textwidth]{reconstruct.34S/for_minus_wind}
    \caption{34S: Absolute error between the full forward model and the reconstruction only using wind. Reconstruction is with only December sensitivity so that no gaps exist and we can look at the frequency spectrum in the next plot. }
    \label{fig:windRecError}
   \end{figure}

%   \begin{figure}
%    \centering
%    \includegraphics[width=.7\textwidth]{reconstruct.34S/freq_for_minus_wind}
%    \caption{34S: Frequency domain of absolute error between the full forward model and the reconstruction only using wind. Reconstruction only with December sens.}
%    \label{fig:freqWindRecError}
%   \end{figure}

   \begin{figure}
    \centering
    \includegraphics[width=.7\textwidth]{reconstruct.34S/monthly/norm_error}
   ...
   \begin{figure}
    \centering
    \includegraphics[width=.7\textwidth]{reconstruct.26N/for_minus_wind}
    \caption{26N: Absolute error between the full forward model and the reconstruction only using wind.}
    \label{fig:windReconstruct26N}
   \end{figure}
 ...
 \end{enumerate}
  ...
  The contribution to $\delta\mathcal{J}(t)$ from heat and salt flux show a very different behavior than in Pillar \cite{pillar}. In the Pillar paper there seems to be a roughly decadal mode appearing from heat flux reconstruction, with very little variation due to salt flux. In my reconstructions, I get strictly oscillatory behavior on roughly at annual time scales. 
